...
public class SmithWilsonDb extends Irmodel {
...
//		SmithWilson sw = new SmithWilson(LocalDate.of(2017, 12, 31), ts, CMPD_MTD_DISC, false, 4.5, 60, 100, 1);
		...
		SmithWilsonDb sw = new SmithWilsonDb(LocalDate.of(2017, 12, 31), curveList, CMPD_MTD_DISC, false, 4.5, 60, 100, 1);		
...
//		log.info("{}", sw.getAlphaApplied());		
	...
	//TODO; 
	public SmithWilsonDb(LocalDate baseDate, List<IrCurveHis> irCurveHisList,                boolean isRealNumber, double ltfr, int ltfrT, int prjYear) {
		this(baseDate, irCurveHisList, CMPD_MTD_DISC, isRealNumber, ltfr, ltfrT, prjYear, 20, 1);		
	}	

	public SmithWilsonDb(LocalDate baseDate, List<IrCurveHis> irCurveHisList, char cmpdType, boolean isRealNumber, double ltfr, int ltfrT, int prjYear, int dayCountBasis) {
		this(baseDate, irCurveHisList, cmpdType     , isRealNumber, ltfr, ltfrT, prjYear, 20, dayCountBasis);		
	}
	
	public SmithWilsonDb(LocalDate baseDate, List<IrCurveHis> irCurveHisList, char cmpdType, boolean isRealNumber, double ltfr, int ltfrT, int prjYear, int alphaItrNum, int dayCountBasis) {				
		super();		
	...
	public SmithWilsonDb(LocalDate baseDate, Map<Double, Double> termStructure,                boolean isRealNumber, double ltfr, int ltfrT, int prjYear) {
		this(baseDate, termStructure,  CMPD_MTD_DISC, isRealNumber, ltfr, ltfrT, prjYear, 20, 1);		
	}

	public SmithWilsonDb(LocalDate baseDate, Map<Double, Double> termStructure, char cmpdType, boolean isRealNumber, double ltfr, int ltfrT, int prjYear, int dayCountBasis) {
		this(baseDate, termStructure,  cmpdType     , isRealNumber, ltfr, ltfrT, prjYear, 20, dayCountBasis);		
	}	
	
	public SmithWilsonDb(LocalDate baseDate, Map<Double, Double> termStructure, char cmpdType, boolean isRealNumber, double ltfr, int ltfrT, int prjYear, int alphaItrNum, int dayCountBasis) {
		super();		
		this.baseDate = baseDate;		
	...
	public List<IrmodelResultDb> getIrmodelResultList() {
...
//		return this.resultList;
		return swProjectionList(smithWilsonAlphaFinding());
	}
	
	
	public Map<String, IrmodelResultDb> getIrmodelResultMap() {
...
//			this.resultMap.putAll(swProjectionMap(smithWilsonAlphaFinding()));
			return swProjectionMap(smithWilsonAlphaFinding());
		}		
...
//		log.info("alphaOpt: {}", round(0.5 * (alphaMaxApplied + alphaMinApplied), this.decimalDigit));
	...
	private List<IrmodelResultDb> swProjectionList(double alpha) {
		
		List<IrmodelResultDb> irResultlList = new ArrayList<IrmodelResultDb>();
		...
		for(int i=0; i<this.prjYearFrac.length; i++) {
			...
			IrmodelResultDb irResult = new IrmodelResultDb();
...
//			irResult.setMatDate(prjDate[i]);
	...
	private Map<String, IrmodelResultDb> swProjectionMap(double alpha) {
		
		Map<String, IrmodelResultDb> swModelMap = new TreeMap<String, IrmodelResultDb>();
		smithWilsonZeta(alpha);
