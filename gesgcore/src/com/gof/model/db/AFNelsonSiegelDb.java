...
public class AFNelsonSiegelDb extends IrmodelDb {		
...
//	}	
	...
	// (2021.06.11) add argument (double[] optParas, double[] optLSC)
	public AFNelsonSiegelDb(LocalDate baseDate, String mode, double[] inputParas, double[] optParas, double[] optLSC, List<IrCurveHis> iRateHisList, List<IrCurveHis> iRateBaseList, boolean isRealNumber, char cmpdType, double dt, double initSigma, int dayCountBasis,
		...
		this.baseDate      = baseDate;		                      // 2020-12-31
		this.mode          = mode;								  // "AFNS"
		this.inputParas    = inputParas;		                  // null
		
		// (2021.06.11) add this
		this.optParas = optParas;			
		this.optLSC   = optLSC;
		
		this.setTermStructureHis(iRateHisList, iRateBaseList);    // curveHistList, curveBaseList
		this.isRealNumber  = isRealNumber;                        // true
		this.cmpdType      = cmpdType;		                      // "D"
		this.dt            = dt;	                              // 1/52
		this.initSigma     = initSigma;                           // 0.05
		this.dayCountBasis = dayCountBasis;                       // 1
		this.ltfrL         = ltfrL;                               // 0.052
		this.ltfrA         = ltfrA;                               // 0.052
		this.ltfrT         = ltfrT;                               // 60
		this.liqPrem       = liqPrem;                             // 0.032
		this.term          = term;                                // 1/12
		this.minLambda     = minLambda;                           // 0.05
		this.maxLambda     = maxLambda;                           // 2.00
		this.nf            = nf;                                  // 3
		this.prjYear       = prjYear;                             // 140
		this.accuracy      = accuracy;                            // 0.0000000001
		this.itrMax        = itrMax;                              // 100
		this.confInterval  = confInterval;                        // 0.995
		this.epsilon       = epsilon;                             // 0.001
		this.setIrmodelAttributes();
	}
	
	public AFNelsonSiegelDb(LocalDate baseDate, String mode, double[] inputParas, List<IrCurveHis> iRateHisList, List<IrCurveHis> iRateBaseList, boolean isRealNumber, char cmpdType, double dt, double initSigma, int dayCountBasis,
            double ltfrL, double ltfrA, int ltfrT, double liqPrem, double term, double minLambda, double maxLambda, int nf, int prjYear, double accuracy, int itrMax, double confInterval, double epsilon) {		
...
//		setIrmodelAttributes();
	...
	private void kalmanFiltering() {
		if (this.optLSC == null | this.optParas == null) {
...
//		log.info("step: {}", step);
	...
	private void afnsShockGenerating() {
		...
		////////////////////////////////////
		this.optParas = new double[] {
			0.3787450166895335,
			0.03417275018241243,
			-0.015375558965320078,
			-0.007205530906623649,
			0.16213754898234956,
			0.5772838304982371,
			0.6720135979580237,
			0.005840197287140233,
			-0.005761256848609649,
			0.0024224721115505823,
			0.0030254991008841882,
			-0.001049859557673153,
			0.0073055466811602805,
			0.22753826069381988
		};
		
		this.optLSC = new double[] {
			0.020803931014466922,
			-0.007890691502394761,
			-0.007683667051499556
		};
		////////////////////////////////////
...
//		SimpleMatrix NTN        = N.transpose().mult(N); 	//for IAIS(International Association of Insurance Supervisors) 2017 Field Testing Tech. doc(page 201 of 326)
		SimpleMatrix NTN	    = N.mult(N.transpose());	//for KODI Article(2018-2) (page 28) 
...
//		log.info("{}, {}", this.IntShock, spotShock);
		...
		for(int i=0; i<spotShock.numCols(); i++) {
			...
			SmithWilsonDb sw = new SmithWilsonDb(this.baseDate, ts, CMPD_MTD_CONT, true, irDiscToCont(ltfr), this.ltfrT, this.prjYear, 20, this.dayCountBasis);			
			List<IrmodelResultDb> swRslt= sw.getIrmodelResultList();
					
			for(IrmodelResultDb rslt : swRslt) {				
...
//				ir.setRfIr(rslt.getSpotDisc());
				ir.setRiskAdjIr(rslt.getSpotDisc());
//				ir.setRiskAdjIr(0.0);
