...
@Getter
@Setter
public class SmithWilsonKics extends IrmodelHw {
	...
	public SmithWilsonKics(LocalDate baseDate, List<IrCurveHis> irCurveHisList,                boolean isRealNumber, double ltfr, int ltfrT, int prjYear) {
		this(baseDate, irCurveHisList, CMPD_MTD_DISC, isRealNumber, ltfr, ltfrT, prjYear, 1, 100, 1);		
	}	

	public SmithWilsonKics(LocalDate baseDate, List<IrCurveHis> irCurveHisList, char cmpdType, boolean isRealNumber, double ltfr, int ltfrT, int prjYear, int dayCountBasis) {
		this(baseDate, irCurveHisList, cmpdType     , isRealNumber, ltfr, ltfrT, prjYear, 1, 100, dayCountBasis);		
	}
	
	public SmithWilsonKics(LocalDate baseDate, List<IrCurveHis> irCurveHisList, char cmpdType, boolean isRealNumber, double ltfr, int ltfrT, int prjYear, int prjInterval, int alphaItrNum, int dayCountBasis) {				
		super();		
...
//			log.info("prjDate: {}, prjYearFrac: {}", prjDate[i] ,prjYearFrac[i]);
	...
	private double smithWilsonAlphaFinding() {
		...
		for(int i=0; i<this.alphaItrNum; i++) {
			...
			this.alphaApplied  = round(0.5 * (this.alphaMaxApplied + this.alphaMinApplied), this.decimalDigit);
			this.smithWilsonZeta(this.alphaApplied, i);			
//			log.info("i:{}, alphaMax:{}, alphaMin:{}, alphaOpt: {}", i, alphaMaxApplied, alphaMinApplied, round(0.5 * (alphaMaxApplied + alphaMinApplied), this.decimalDigit));	
		}
//		log.info("alphaOpt: {}", round(0.5 * (alphaMaxApplied + alphaMinApplied), this.decimalDigit));
		return round(0.5 * (this.alphaMaxApplied + this.alphaMinApplied), this.decimalDigit);
	}
...
//		}
		...
		/**
		 * for fwdDisc[0] = f(0, 0, 1) = r(0, 1)), fwdDisc[719]
		 */
		for(int i=0; i<this.prjYearFrac.length-1; i++) {			
			priceZcb[i] = priceCol.getEntry(i,0);
			spotCont[i] = -1.0 / this.prjYearFrac[i] * Math.log(priceZcb[i]);
