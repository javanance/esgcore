...
@Setter
@Slf4j
public abstract class IrmodelHw {
...
//		this.tenor = irCurveHis.stream().map(s -> Double.parseDouble(s.getMatCd().substring(1, 5)) / 12.0).mapToDouble(Double::doubleValue).toArray();
		for(IrCurveHis curve : irCurveHisList) this.termStructureBase.put(Double.parseDouble(curve.getMatCd().substring(1, 5)) / MONTH_IN_YEAR, curve.getIntRate());		
		setTermStructureBase(this.termStructureBase);							
	}
	

	public void setTermStructureYtm(List<IrCurveYtmHis> ytmCurveHisList) {	

		for(IrCurveYtmHis curve : ytmCurveHisList) this.termStructureBase.put(Double.parseDouble(curve.getMatCd().substring(1, 5)) / MONTH_IN_YEAR, curve.getYtmRate());		
		setTermStructureBase(this.termStructureBase);							
	}	
...
//		 (3,1), (3,2),  |  (3,1), (3,2),  |  (2,1), (2,2)		
    ...
    protected static LocalDate stringToDate(String dateString) {
...
//			log.info("{}", date[i]);
	...
	public static String[][] doubleToString2D(double[][] array) {
		...
		for(int i=0; i<array.length; i++) {
			Double[] vec = ArrayUtils.toObject(array[i]);
			strArray[i] = ArrayUtils.toStringArray(vec);			
		}		
		return strArray;
	}
	...
	protected static double vectMean(double[] vec) {
	...
	protected static double vectSd(double[] vec) {
