...
@Setter
@Slf4j
public class SmithWilson extends IrmodelHw {
...
//		SmithWilson sw = new SmithWilson(LocalDate.of(2017, 12, 31), curveList, CMPD_MTD_DISC, false, 4.5, 60, 100, DCB_ACT_365);		
		SmithWilson sw = new SmithWilson(LocalDate.of(2017, 12, 31), curveList, CMPD_MTD_DISC, false, 4.5, 60, 100, 3, 20, DCB_MON_DIF);
...
//		sw.getSmithWilsonResultMap().entrySet().stream().filter(s-> Double.parseDouble(s.getKey().substring(1, 5)) >= 1190).forEach(s->log.info("{}, {}, {}", s.getKey(), s.getValue().getSpotDisc(), s.getValue().getFwdDisc()));
	...
	public SmithWilson(LocalDate baseDate, List<IrCurveHis> irCurveHisList, char cmpdType, boolean isRealNumber, double ltfr, int ltfrT, int prjYear, int prjInterval, int alphaItrNum, int dayCountBasis) {				
		super();		
	...
	public SmithWilson(LocalDate baseDate, Map<Double, Double> termStructure, char cmpdType, boolean isRealNumber, double ltfr, int ltfrT, int prjYear, int prjInterval, int alphaItrNum, int dayCountBasis) {
		super();		
		this.baseDate = baseDate;		
