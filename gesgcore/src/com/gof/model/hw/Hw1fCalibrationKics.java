...
@Setter
@Slf4j
public class Hw1fCalibrationKics extends IrmodelHw {	
...
//	}	
	...
	public Hw1fCalibrationKics(String bssd, List<IrCurveHis> iRateBaseList, List<SwpnVolInfo> swpnVolInfo, int[] alphaPiece, int[] sigmaPiece, double[] initParas , int prjYear, double accuracy) {				
...
//		this.cmpdType = CMPD_MTD_CONT;
	...
	public Hw1fCalibrationKics(LocalDate baseDate, List<IrCurveHis> iRateBaseList, List<SwpnVolInfo> swpnVolInfo, int[] alphaPiece, int[] sigmaPiece, double[] initParas, int freq, double notional,
			               char cmpdType, boolean isRealNumber, int prjYear, int prjInterval, int dayCountBasis, int itrMax, double accuracy) {				
		super();		
...
//		checkSwpnVolDiff();
		...
		for(int i=0; i<this.sigmaPiece.length+1; i++) {			
			...
			int outerPiece = (int) this.lastLiquidPoint * MONTH_IN_YEAR;
			...
			if(i<this.sigmaPiece.length) {				
				...
				param.setAlpha(Math.max(this.optParas[0], 1e-4));
				param.setSigma(Math.max(this.optParas[i+2], 1e-5));
			}
			else {
				param.setMonthSeq(outerPiece);
				param.setMatCd(String.format("%s%04d", TIME_UNIT_MONTH, outerPiece));
				param.setAlpha(Math.max( (this.alphaPiece[0] <= outerPiece ? this.optParas[1] : this.optParas[0]), 1e-4));
				param.setSigma(Math.max( (this.sigmaPiece[0] <= outerPiece ? this.optParas[7] : this.optParas[2]), 1e-5));
			}			
			hw1fParam.add(param);
		}		
		return hw1fParam;		
...
//		swRsltList.stream().filter(s-> Double.parseDouble(s.getMatCd().substring(1, 5)) % 12 == 0)
